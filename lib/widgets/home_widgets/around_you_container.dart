...
class _AroundYouContainerState extends State<AroundYouContainer> {
  ...
  Widget _getCurrentCityName(BuildContext context) {
    ...
    if (CurrentPositionService().permission ==
        CurrentPositionPermission.granted) {
      return FutureBuilder(
          future: CurrentPositionService().getCurrentCityName(),
          builder: (context, snapshot) {
            return Text(snapshot.data ?? localization.gettingLocation);
          });
    } else if (CurrentPositionService().permission ==
        CurrentPositionPermission.denied) {
      return Text(localization.no_permission);
    } else if (CurrentPositionService().permission ==
        CurrentPositionPermission.serviceDisabled) {
  ...
  Future<QueryBatch<Item>> _getAroundYouItems(
      [DocumentSnapshot? startAfterDoc]) async {
    _boundingBoxingQuery = await getItemsByGeoPoint(
        CurrentPositionService().geoPoint!,
        _boundingBoxingQuery.currentDistance,
        startAfterDoc);
                    ...
                    onPressed: () {
                      ...
                      Navigator.pushNamed(context, ItemGridScreen.id,
                          arguments: ItemGridScreenArguments(
                              localization.aroundYou, _getAroundYouItems));
                    },
                    child: Text(
